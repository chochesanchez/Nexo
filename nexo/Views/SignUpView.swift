...
struct SignUpView: View {
    ...
    var body: some View {
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    PhotosPicker(selection: $avatarItem, matching: .images) {
                        HStack(spacing: 14) {
                            ZStack {
                                Circle().fill(Color.nexoMint).frame(width: 52, height: 52)
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                ...
                                Text("Opcional")
                                    .font(.system(size: 12)).foregroundStyle(Color(uiColor: .tertiaryLabel))
                            }
                            Spacer()
                    ...
                    formSection("Datos personales") {
                        ...
                        formDivider
                        formField("Edad", text: $edadText).keyboardType(.numberPad)
                    }
                    .padding(.bottom, Sp.md)
                    ...
                    formSection("Contacto") {
                        ...
                        formDivider
                        formField("Teléfono (opcional)", text: $telefono).keyboardType(.phonePad)
                    }
                    .padding(.bottom, Sp.md)
                    ...
                    .disabled(isLoading)
                    .padding(.horizontal, Sp.lg).padding(.bottom, 16)
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarLeading) {
                // Patrón consistente: chevron.left + "Regresar" en nexoForest
    ...
    private var formDivider: some View {
        Rectangle().fill(Color(uiColor: .separator)).frame(height: 0.5).padding(.leading, Sp.md)
    }

    private func formField(_ label: String, text: Binding<String>) -> some View {
        TextField(label, text: text)
            .font(.system(size: 15)).foregroundStyle(Color(uiColor: .label))
            .padding(.horizontal, Sp.md).frame(height: 50).tint(Color.nexoBrand)
    }
    ...
    private func createAccount() {
        localError = nil; auth.errorMessage = nil
        guard !nombre.isEmpty, !apellido.isEmpty, !email.isEmpty, !password.isEmpty else {
            localError = "Completa todos los campos."; return
        }
        guard let edad = Int(edadText), edad >= 18 else {
            localError = "Debes tener 18 años o más."; return
        }
        Task {
            isLoading = true
            await auth.signUp(nombre: nombre, apellido: apellido, email: email,
                              telefono: telefono, edad: edad, password: password, avatarData: avatarData)
            isLoading = false
        }
    }
}
